...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # otherwise False.
        ############
        # 1) transform from vehicle to sensor coordinates
        pos_veh = np.ones((4, 1))  # make komogenious coordinates from x
        pos_veh[0:3] = x[0:3]
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            # - return h(x)
            ############
            pos_veh = np.ones((4, 1))  # homogeneus coordinates
            pos_veh[0:3] = x[0:3]
            pos_sens = self.veh_to_sens * pos_veh
            
            if pos_sens[0] == 0:
                raise RuntimeError('Jacobian not defined for pos_sens[0]=0!')
...
class Measurement:
    ...
    def __init__(self, num_frame: int, z, sensor: Sensor):
