...
def load_configs_model(model_name='darknet', configs=None):
    ...
    # GPU vs. CPU
    # configs.no_cuda = False # True # if true, cuda is not used
...
def create_model(configs):
    ...
    if (configs.arch == 'darknet') and (configs.cfgfile is not None):
        ...
        # model = fpn_resnet.get_pose_net(configs.num_layers, configs.heads, configs.head_conv, False)  # False = do not load model weights from url        
...
def detect_objects(input_bev_maps, model, configs):
    ...
    addjust_y_to_zero = (configs.lim_y[1] - configs.lim_y[0]) / 2

    ## step 2 : loop over all detections
    for det in detections:
        score, bev_x, bev_y, z, h, bev_w, bev_l, yaw = det   # we need to change x and y coordinates in next step
        ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure        
        x = bev_y / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0])
        y = bev_x / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0]) - addjust_y_to_zero
        ...
        # append only if object is whitin limits
        if (configs.lim_x[0] <= x <= configs.lim_x[1]) and (configs.lim_y[0] <= y <= configs.lim_y[1]) and (configs.lim_z[0] <= z <= configs.lim_z[1]):
