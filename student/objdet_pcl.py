...
# my helper (display image for comparing image to pcl)
...
def show_pcl(pcl, frame=None):
    ...
    vis.create_window(window_name="Point Cloud", width=2560, height=2160, left=50, top=50, visible=True)
    
    # step 2 : create instance of open3d point-cloud class
    pcd = o3d.geometry.PointCloud()

    # step 3 : set points in pcd instance by converting the point-cloud into 3d vectors (using open3d function Vector3dVector)    
    # idxs = (pcl[:,0] > 0) & (pcl[:,1] > -20) & (pcl[:,1] < 20)
    # pcd.points = o3d.utility.Vector3dVector(pcl[idxs, :3])
    pcd.points = o3d.utility.Vector3dVector(pcl[:, :3]) # use whole pcl and open3d viewer transformations
...
def show_range_image(frame, lidar_name, crop_90=False):
    ...
    # described only in project rubric: "Crop range image to +/- 90 deg. left and right of the forward-facing x-axis" begin
    if crop_90:
        ri_center = int(ri.shape[1]/2)
        deg_90 = int(ri.shape[1]/4)
        ri = ri[:, ri_center-deg_90:ri_center+deg_90, :]
    # described only in project rubric: "Crop range image to +/- 90 deg. left and right of the forward-facing x-axis" end
    ri_range = ri[:,:,0]
    ri_intensity = ri[:,:,1]
    ...
    # I used heuristic from provious lessons
...
def bev_from_pcl(lidar_pcl, configs, show=False):
    ...
    lidar_pcl_cpy[lidar_pcl_cpy[:, 3] > 1.0, 3] = 1.0  # ???  clip intensitiy values if intensity > 1 => set to 1
    idx_intensity = np.lexsort((-lidar_pcl_cpy[:, 3], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
    lidar_pcl_cpy = lidar_pcl_cpy[idx_intensity]
    
    idx_top = np.lexsort((-lidar_pcl_cpy[:, 2], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
    lidar_pcl_top = lidar_pcl_cpy[idx_top]
    ...
    if False:
        ...
        #intensity map is normalized to [0,1] and I watn to show values between [0, 255]
        # y = 365
        # x = 295           
        # part_int_map = (intensity_map[y:y+20,x:x+20] * 255).astype(np.uint8)
        # _, ax = plt.subplots(figsize=(18,18))         
        # sns.heatmap(part_int_map, cmap='Greys_r', ax=ax, annot=True, fmt="d", cbar=False).set(title=f'Zoomed In 20x20 at ({x},{y})')
        # plt.show()
    #######
    ####### ID_S2_EX2 END ####### 
    ...
    if False:
        ...
        # y = 365
        # x = 295           
        # part_height_map = (height_map[y:y+20,x:x+20] * 255).astype(np.uint8)
        # _, ax = plt.subplots(figsize=(18,18))         
        # sns.heatmap(part_height_map, cmap='Greys_r', ax=ax, annot=True, fmt="d", cbar=False).set(title=f'Zoomed In 20x20 at ({x},{y})')
        # plt.show()
    #######
    ####### ID_S2_EX3 END #######
