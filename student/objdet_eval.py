...
import numpy as np
import matplotlib
# could not install wxpython. That's why next line is commented out
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for det in detections:
                ...
                cl, x, y, z, h, w, l, yaw = det
                
                det_box = tools.compute_box_corners(x, y, w, l, yaw)
                ## step 4 : compute the center distance between label and detection bounding-box in x, y, and z
                dx = label.box.center_x - x
                dy = label.box.center_y - y
                dz = label.box.center_z - z
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                det_poly = Polygon(det_box)
                intersect = label_poly.intersection(det_poly).area
...
def compute_performance_stats(det_performance_all, configs_det):
    ...
    # pos_neg indixes: (0, 1, 2, 3) = (all_positives, true_positives, false_negatives, false_positives)
    positives, true_positives, false_negatives, false_positives = pos_negs.sum(axis=0)
    ...
    for tuple in center_devs:
        # convert from tensor to numpy types (because of ax.hist don't like list of tensores)
